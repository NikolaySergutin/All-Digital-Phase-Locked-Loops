...
module Int_N_ADPLL_NCO_TDC_tb;
  ...
  // Instantiate PLL_MODEL
  Int_N_ADPLL_NCO_TDC #(.DIV_WIDTH(6)) Int_N_ADPLL_NCO_TDC_uut (
    .REF_rsvd(REF_rsvd),
    .MUL_VAL(40),
    .EN(EN),  
    .CLK(CLK),
  ...
  initial begin
    ...
    fd = $fopen("Int_N_ADPLL_NCO_TDC_freq_log.txt", "w");
    if (fd == 0) begin
        $display("ERROR: cannot open file Int_N_ADPLL_NCO_TDC_freq_log.txt!");
        $finish;
    end
    $fwrite(fd, "TIME(ns),FREQ(Hz)\n");
    $fwrite(fd, "%0t,%d\n", 0, 0);
    ...
    #8000000;  //finish file 
    $fclose(fd);
    $display("Int_N_ADPLL_NCO_TDC_freq_log.txt saved.");
  end
